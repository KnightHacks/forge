    ...
    try {
      ...
      for (const perm of userPermissions) {
        if (!discordRoleIds.includes(perm.discordRoleId)) {
          console.log(
            `Removing role "${perm.roleName}" from user ${user.name} (${user.discordUserId}) - not on Discord`,
          );
          await db
            .delete(Permissions)
      ...
      for (const role of linkedRoles) {
        ...
        console.log(
          `Adding role "${role.name}" to user ${user.name} (${user.discordUserId}) - found on Discord`,
        );
