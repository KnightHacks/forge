  ...
  try {
    ...
    void emailService
      ...
      .then((config) => {
        ...
        console.log(
          `Starting email queue processor with schedule: ${cronSchedule}`,
        );
        ...
        cron.schedule(cronSchedule, async () => {
          try {
            console.log(
              `[CRON] Email queue job triggered at ${new Date().toISOString()}`,
            );
    ...
    if (err instanceof Error) {
      ...
      console.error(
        "An unknown error occurred starting email queue processor:",
        err,
      );
    }
  }
...
async function processEmailQueue(
  ...
  console.log(
    `Daily limit: ${limit}, Current count: ${count}, Remaining: ${remaining}`,
  );
  ...
  if (emailsToSend.length > 0) {
    ...
    emailsToSend.forEach((e, index) => {
      const scheduledInfo = e.scheduled_for
        ? `(was scheduled for ${e.scheduled_for.toISOString()})`
        : "(immediate)";
      console.log(
        `  ${index + 1}. [${e.priority.toUpperCase()}] ${e.subject} (${e.to}) - ${scheduledInfo}`,
      );
    });
  }
  ...
  for (const email of emailsToSend) {
    ...
    if (!emailService.canSendEmail(email)) {
      console.log(
        `Email ${email.id} [${email.priority.toUpperCase()}] "${email.subject}" skipped due to blacklist rules`,
      );
      skippedCount++;
      continue;
    }
    ...
    if (email.batch_id) {
      ...
      const batchResult = await emailService.processBatch(
        email.batch_id,
        remaining - sentCount,
      );
      ...
      console.log(
        `Batch ${email.batch_id}: sent ${batchResult.sent}, scheduled ${batchResult.scheduled}`,
      );
    } else {
      // Send individual email
      console.log(
        `Sending email ${email.id} [${email.priority.toUpperCase()}] "${email.subject}" to ${email.to}`,
      );
  ...
  console.log(
    `Email queue processing complete: ${sentCount} sent, ${skippedCount} skipped, ${failedCount} failed`,
  );
