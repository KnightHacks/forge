...
export function HackerFormPage({
  ...
  useEffect(() => {
    if (
      !shouldRedirectExistingApplication({
        applicationSubmitted,
        existingApplication: Boolean(existingApplication),
  ...
  const requiredAgreementsAccepted = haveRequiredAgreements(
    applicationAgreements,
    agreementChoices,
    agreementAcceptances,
  );
  ...
  const goToStep = useCallback(
    async (
      ...
      if (nextStep > activeStep) {
        ...
        if (currentStep.id === "tosAccepted" && !requiredAgreementsAccepted) {
          setTosError(true);
          return false;
        }
    ...
    <Form {...form}>
      ...
      <form
        ...
        onSubmit={form.handleSubmit(async (values) => {
          ...
          if (!requiredAgreementsAccepted) {
            setTosError(true);
            toast.error("Please accept every required agreement.");
            setLoading(false);
            return;
          }
          ...
          try {
            ...
            if (resumeResult.resumeError) {
              toast.warning(
                resumeResult.resumeError instanceof Error
                  ? `Your application was submitted, but the resume did not upload: ${resumeResult.resumeError.message}`
                  : "Your application was submitted, but the resume did not upload. Add it from your profile before applications close.",
              );
            }
            setLoading(false);
            router.replace("/dashboard");
                    ...
                    >
                      ...
                      >
                        {applicationAgreements.length === 0 ? (
                          <p className={checkboxLabelClassName}>
                            There are no application agreements to review.
                          </p>
                        ) : null}
                        {applicationAgreements.map((definition) => {
                          const accepted = isAgreementAccepted(definition.id);
                          const showError =
                          ...
                          return (
                            <div
                              ...
                              className={cn(
                                "flex flex-row items-start gap-3 space-y-0",
                                showError && agreementErrorRowClassName,
                              )}
                            >
                              <Checkbox
                                ...
                                className={cn(
                                  checkboxClassName,
                                  showError && agreementErrorCheckboxClassName,
                                )}
                                onCheckedChange={(value) => {
                                  ...
                                  if (value === true) setTosError(false);
                                }}
                              />
