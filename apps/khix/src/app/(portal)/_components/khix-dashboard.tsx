...
export function KhixDashboard({ sessionUser }: KhixDashboardProps) {
  ...
  const handleConfirm = async (
    ...
    try {
      await confirmAttendance(agreements);
    ...
    >
      ...
      {scheduleQuery.isError ? (
        <ScheduleState
          ...
          copy="The event list could not be loaded. Check your connection and try once more."
          title="The trail went quiet."
        />
      ) : null}
      {scheduleQuery.isSuccess && schedule.length === 0 ? (
        <ScheduleState
          copy="The organizers are still arranging the trail. Check back soon for the first events."
...
function ScheduleExperience({
  ...
  events: ReturnType<typeof useHackerDashboardFlow>["schedule"];
  timeZone: string;
}) {
  const days = groupScheduleByDay(events, timeZone);

  return (
    <section
      className={styles.scheduleExperience}
    ...
    >
      <header className={styles.scheduleHero}>
        ...
        <p className={styles.scheduleIntro}>
          Workshops, meals, ceremonies, and side quests—all in hackathon-local
          time.
        </p>
      </header>
      ...
      <div className={styles.scheduleDays}>
        {days.map((day, dayIndex) => (
          <section
            ...
            aria-labelledby={`khix-schedule-${day.key}`}
          >
            <header className={styles.scheduleDayHeader}>
              <span className={styles.scheduleDayNumber} aria-hidden="true">
                {String(dayIndex + 1).padStart(2, "0")}
              </span>
              <h2 id={`khix-schedule-${day.key}`}>{day.dateLabel}</h2>
              <span className={styles.scheduleDayCount}>
                {day.events.length}{" "}
              ...
              {day.events.map((event) => {
                ...
                return (
                  <li key={event.id} className={styles.scheduleItem}>
                    ...
                    <div className={styles.scheduleEventDetails}>
                      <h3 className={styles.scheduleTitle}>{event.name}</h3>
                      {description ? (
                        <p className={styles.scheduleCopy}>{description}</p>
                      ) : null}
                    ...
                    <div className={styles.scheduleMeta}>
                      {event.purpose === "primary_check_in" ? (
                        <span className={styles.schedulePurpose}>Check-in</span>
                      ) : null}
                      {tag ? (
                        <span className={styles.scheduleTag}>{tag}</span>
                      ) : null}
                      <span className={styles.schedulePoints}>
                        {event.points} {event.points === 1 ? "pt" : "pts"}
                      </span>
                    </div>
                  </li>
                );
...
function ScheduleState({
  ...
  return (
    <section className={styles.scheduleState} aria-live="polite">
      ...
      <p>{copy}</p>
      {action ? (
        <div className={styles.scheduleStateAction}>{action}</div>
      ) : null}
    </section>
  );
}
  ...
  return (
    <section
      className={joinClasses(
        ...
        styles.scheduleSkeleton,
      )}
      aria-label="Loading event schedule"
      aria-busy="true"
    >
      <div className={styles.scheduleSkeletonHero} />
      <div className={styles.scheduleSkeletonDay}>
        <div className={styles.scheduleSkeletonHeading} />
        {Array.from({ length: 4 }, (_, index) => (
          <div key={index} className={styles.scheduleSkeletonRow} />
        ))}
      </div>
    </section>
  );
...
function getProfileFormDefaults(
  ...
  return {
    ...
    isFirstTime: participant.isFirstTime,
    foodAllergies: participant.foodAllergies,
    agreesToReceiveEmailsFromMLH: participant.agreesToReceiveEmailsFromMLH,
    agreesToMLHCodeOfConduct: participant.agreesToMLHCodeOfConduct,
    agreesToMLHDataSharing: participant.agreesToMLHDataSharing,
  };
}
...
function ProfileSection({
  ...
  const isAgreementAccepted = (definitionId: string) =>
    agreementIsAccepted(definitionId, agreementChoices, agreementAcceptances);
  const agreementInputs = buildDisplayedAgreementInputs(
    agreements,
    agreementChoices,
    agreementAcceptances,
  );
  ...
  return (
    ...
    >
      <Form {...form}>
        <form
          ...
          onSubmit={form.handleSubmit(async (values) => {
            ...
            if (
              ...
              toast.error("Accept every required agreement before saving.");
              return;
            }
            setIsSaving(true);
            try {
              const resumeResult = await persistBeforeOptionalResumeUpload({
                file: values.resumeUpload?.[0],
                persist: async () =>
                  updateProfile(
                    {
                      ...
                      resumeUrl: values.resumeUrl || participant.resumeUrl,
                    },
                    agreementInputs,
                  ).then(() => undefined),
                uploadResume,
              ...
              if (resumeResult.resumeError) {
                toast.warning(
                  getToastErrorMessage(
                    ...
                    "Your profile was saved, but the resume did not upload. Try the resume again.",
                  ),
                );
                return;
              }
        ...
        >
          {!editable ? (
            <div className="mb-4 rounded-lg border border-amber-300/30 bg-amber-200/10 px-4 py-3 text-sm text-amber-50">
              Profile editing is currently closed for this application. Your
              saved information remains available below.
            </div>
          ) : null}
          <fieldset className="contents" disabled={!editable || disabled}>
            <div className={styles.profileFormShell}>
              ...
              <div className={styles.profileEditor}>
                ...
                <div className={styles.profileFormContent}>
                  ...
                  <ProfileFormBlock
                    copy="Review the current agreements configured for this hackathon."
                    title="Agreements"
                  >
                    <div className={styles.profileConsentList}>
                      ...
                      {agreements.map((definition) => (
                        <div
                          key={definition.id}
                          className={styles.profileConsent}
                        >
                          <Checkbox
                            aria-labelledby={`profile-agreement-${definition.id}`}
                            checked={isAgreementAccepted(definition.id)}
                            className={styles.profileCheckbox}
                            onCheckedChange={(value) =>
                              setAgreementChoices((current) => ({
                                ...current,
                                [definition.id]: value === true,
                              }))
                            }
                          />
                          <div className={styles.profileConsentCopy}>
                            <div
                              ...
                              id={`profile-agreement-${definition.id}`}
                            >
                              {definition.title}{" "}
                              {definition.required ? (
                                <RequiredMark />
...
function StatusAction({
  ...
  const [agreementChoices, setAgreementChoices] = useState<
    Record<string, boolean>
  >({});
  const requiredAccepted = requiredAgreementsAccepted(
    agreements,
    agreementChoices,
  );
          ...
          <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-xl">
            ...
            <DialogFooter>
              <Button
                ...
                onClick={() => {
                  void onConfirm(
                    buildDisplayedAgreementInputs(agreements, agreementChoices),
                  )
  ...
  if (status === "confirmed" || status === "checkedin") {
    return (
      <>
        ...
        />
        {status === "confirmed" ? (
          <WithdrawalDialog disabled={actionPending} onWithdraw={onWithdraw} />
        ) : null}
      </>
    );
  }
