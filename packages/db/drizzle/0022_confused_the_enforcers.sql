...
--> statement-breakpoint
ALTER TABLE "knight_hacks_form_attachment" ADD COLUMN "purpose" text DEFAULT 'response' NOT NULL;--> statement-breakpoint
UPDATE "knight_hacks_form_attachment" AS attachment
SET "purpose" = 'instruction'
FROM "knight_hacks_form_schemas" AS form_schema
WHERE attachment."form_id" = form_schema."id"
	AND attachment."response_id" IS NULL
	AND EXISTS (
		SELECT 1
		FROM jsonb_array_elements(
			COALESCE(form_schema."form_data"->'instructions', '[]'::jsonb)
		) AS instruction
		WHERE instruction->>'attachmentId' = attachment."id"::text
	);--> statement-breakpoint
...
ALTER TABLE "knight_hacks_form_attachment" ADD CONSTRAINT "knight_hacks_form_attachment_purpose_check" CHECK ("knight_hacks_form_attachment"."purpose" IN ('instruction', 'response'));
--> statement-breakpoint
CREATE FUNCTION "audit_reject_mutation"()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
...
--> statement-breakpoint
CREATE TRIGGER "audit_subject_reject_mutation"
