...
The room timeline spans judging start through end with native vertical scrolling. Cards retain their full size and position as time advances. Horizontal scrolling and left/right buttons navigate rooms, grouped by building and then natural room-number order. Now scrolls to the current time. A live summary counts rooms in a reserved slot, rooms without a current slot, and total scheduled rooms across all filters. Slot occupancy includes setup and teardown; MLH rooms are excluded.
