...
COPY . .
RUN turbo prune @forge/2021 --docker
...
RUN pnpm --filter @forge/2021 build

FROM nginxinc/nginx-unprivileged:1.29-alpine AS runner
ENV PORT=3009
ENV NGINX_ENVSUBST_FILTER=PORT
COPY deploy/nginx/archive.conf.template /etc/nginx/templates/default.conf.template
COPY --from=builder /app/apps/2021/out/ /usr/share/nginx/html/
USER nginx
EXPOSE 3009
STOPSIGNAL SIGQUIT
